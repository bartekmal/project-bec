...
histogramsToDraw = [
    ...
    {'name': 'h101', 'refName': 'h101', 'flagRefInput': 'custom', 'nameEnd': '',
     'dim': '1D', 'type': 'normalised', 'flagBins': 0, 'isHistFullName': True, 'title' : ';#font[12]{N}_{VELO};Entries / (1.0)'},
...
histogramsSlopes = [
    # {'name': 'h3053', 'refName': 'h3053', 'flagRefInput': 'custom', 'nameEnd': '',
    #  'dim': '1D', 'type': 'std', 'flagBins': 0, 'isHistFullName': False, 'title': ';#Delta#font[12]{t_{x}};Entries / (0.01 #times 10^{-3})'},
    {'name': 'h3053_restrictedQ', 'refName': 'h3053_restrictedQ', 'flagRefInput': 'custom', 'nameEnd': '',
     'dim': '1D', 'type': 'std', 'flagBins': 1, 'isHistFullName': False, 'title': ';#Delta#font[12]{t_{x}};Entries / (0.01 #times 10^{-3})'},
    {'name': 'h3053_restrictedQ', 'refName': 'h3053_restrictedQ', 'flagRefInput': 'custom', 'nameEnd': '',
     'dim': '1D', 'type': 'std', 'flagBins': 1, 'isHistFullName': False, 'title': ';#Delta#font[12]{t_{x}};Entries / (0.01 #times 10^{-3})', 'logScale' : 'y'},

    # {'name': 'h3054', 'refName': 'h3054', 'flagRefInput': 'custom', 'nameEnd': '',
    #  'dim': '1D', 'type': 'std', 'flagBins': 0, 'isHistFullName': False, 'title': ';#Delta#font[12]{t_{y}};Entries / (0.01 #times 10^{-3})'},
    {'name': 'h3054_restrictedQ', 'refName': 'h3054_restrictedQ', 'flagRefInput': 'custom', 'nameEnd': '',
     'dim': '1D', 'type': 'std', 'flagBins': 1, 'isHistFullName': False, 'title': ';#Delta#font[12]{t_{y}};Entries / (0.01 #times 10^{-3})'},
    {'name': 'h3054_restrictedQ', 'refName': 'h3054_restrictedQ', 'flagRefInput': 'custom', 'nameEnd': '',
     'dim': '1D', 'type': 'std', 'flagBins': 1, 'isHistFullName': False, 'title': ';#Delta#font[12]{t_{y}};Entries / (0.01 #times 10^{-3})', 'logScale' : 'y'},
]
...
def runHistograms(outputFolderName="histograms"):
    ...
    for hist in histogramsToDraw:
        ...
        if 'logScale' in hist:
            logScaleX = (hist['logScale'] == 'x' or hist['logScale'] == 'xy')
            logScaleY = (hist['logScale'] == 'y' or hist['logScale'] == 'xy')
        else:
            logScaleX = False
            logScaleY = False
        ...
        for aJob in jobsToRun:
            ...
            os.system('root -l -b -q \'' + os.getenv('BEC_BASE_CODE_SCRIPTS') + '/drawHistograms.C("{}","{}","{}","{}","{}","{}","{}",{},{},{},{},{},{},{},"{}")\' >> histograms.log'.format(
                aJob['fileMainPath'], aJob['hMainName'], aJob['fileRefPath'], aJob['hRefName'], aJob['hMainNameEnd'], aJob['outputFolder'], aJob['dataTypeRef'], int(
                    isHist1D), int(flagNormalise), int(flagIntegrated), hist['flagBins'], int(hist['isHistFullName']), int(logScaleX), int(logScaleY), histTitle
            ))
