...
inline std::vector<HBT::Units::TH1FloatType> HBT::Histograms::CreateHistogramSeries3000(const std::string &setHeader, const std::string &pairType, const int &currentMultiplicityBin, const int &currentKtBin)
{
  ...
  // slopes
  hSet.push_back(HBT::Units::TH1FloatType(std::string("h" + setHeader + "53" + "_" + strMultiplicity + "_" + strKt).c_str(), std::string("#Deltat_{x} (#Deltat_{y} < 0.5 mrad)" + tmpPairType + ";#Deltat_{x} [rad]; dN/d(#Deltat_{x})").c_str(), 100, -0.0005, 0.0005));
  hSet.push_back(HBT::Units::TH1FloatType(std::string("h" + setHeader + "53_restrictedQ" + "_" + strMultiplicity + "_" + strKt).c_str(), std::string("#Deltat_{x} (#Deltat_{y} < 0.5 mrad) (restricted Q)" + tmpPairType + ";#Deltat_{x} [rad]; dN/d(#Deltat_{x})").c_str(), 100, -0.0005, 0.0005));
  hSet.push_back(HBT::Units::TH1FloatType(std::string("h" + setHeader + "54" + "_" + strMultiplicity + "_" + strKt).c_str(), std::string("#Deltat_{y} (#Deltat_{x} < 0.5 mrad)" + tmpPairType + ";#Deltat_{y} [rad]; dN/d(#Deltat_{y})").c_str(), 100, -0.0005, 0.0005));
  hSet.push_back(HBT::Units::TH1FloatType(std::string("h" + setHeader + "54_restrictedQ" + "_" + strMultiplicity + "_" + strKt).c_str(), std::string("#Deltat_{y} (#Deltat_{x} < 0.5 mrad) (restricted Q)" + tmpPairType + ";#Deltat_{y} [rad]; dN/d(#Deltat_{y})").c_str(), 100, -0.0005, 0.0005));
  // Q distribution
