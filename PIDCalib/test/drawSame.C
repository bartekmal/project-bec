...
void drawSame1(TString f1Name,TString h1Name,TString f2Name,TString h2Name,TString title = "", TString fOut="")
{
  ...
  */
  ...
  tl->AddEntry(h1, "uncorrected" ,"pe");
  tl->AddEntry(h2, "PID-corrected", "l");
...
void drawSame()
{
  TString path = TString("/afs/cern.ch/work/b/bmalecki/analysis/BEC_pPb/PIDCalib/test/HBTAnalysis/");

  TString fName = TString("pPb_MU");

  TString title = TString("pionNN;pionNN;(1/N)(dN/d(pionNN))");
  TString h1Name = TString("h1005");
  TString h2Name = TString("h1032");
  drawSame1(TString(path+fName+".root"),h1Name,TString(path+fName+".root"),h2Name,title,TString(fName+"_"+h1Name));
