...
int main(int argc, char** argv) {
  ...
  int MC = 1;
  TString label = TString("PIDVars");
  TString f1_name;

  if(argc>1)
  {
    if(!strcmp(argv[1], "pPb_MU")){
      f1_name = TString("/eos/lhcb/user/b/bmalecki/BEC_pPb/ntuple/MC_pPb_MU/hbtNTuple_corrected.root");
      label = TString("pPb_MU");
    }
    if(!strcmp(argv[1], "pPb_MD")){
      f1_name = TString("/eos/lhcb/user/b/bmalecki/BEC_pPb/ntuple/MC_pPb_MD/hbtNTuple_corrected.root");
      label = TString("pPb_MD");
    }
    if(!strcmp(argv[1], "Pbp_MU")){
      f1_name = TString("/eos/lhcb/user/b/bmalecki/BEC_pPb/ntuple/MC_Pbp_MU/hbtNTuple_corrected.root");
      label = TString("Pbp_MU");
    }
    if(!strcmp(argv[1], "Pbp_MD")){
      f1_name = TString("/eos/lhcb/user/b/bmalecki/BEC_pPb/ntuple/MC_Pbp_MD/hbtNTuple_corrected.root");
      label = TString("Pbp_MD");
    }
  ...
  TH1D* h101 = new TH1D("h101","PVMult1", 100, 0.0, 100.0);
  ...
  for(ifile = fl.begin(); ifile != fl.end(); ++ifile, ++nFile) {
    ...
    f->ls();
    TTree* HBTTuple = (TTree*)f->Get("HBT");
    ...
    for(int ievt = 0; ievt < nEvts; ievt++) {
      ...
      for(int i = 0; i < N; i++) {
        ...
        // All particles
        h1032->Fill(fabs(piNNcor[i]));
        h1033->Fill(fabs(kNNcor[i]));
        h1034->Fill(fabs(pNNcor[i]));
