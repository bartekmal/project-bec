...
JOB_DIR=${BASE_BEC}/PIDCalib/jobs/default
...
CALIB_FILE=${JOB_DIR}/PIDHists_Strip${STRIPPING}_${MAGNET}.root
URANIA_DIR=${BASE_BEC}/PIDCalib/UraniaDev_v7r0
SCRIPT_DIR=${URANIA_DIR}/PIDCalib/PIDPerfScripts/scripts/python/MCResampling

cd $JOB_DIR
${URANIA_DIR}/run python ${SCRIPT_DIR}/MakePIDdistributionsRunRange.py "$STRIPPING" "$MAGNET" "[K,Mu,Pi,${PROTON_SAMPLE},e]" "[${TUNE}ProbNNK,${TUNE}ProbNNmu,${TUNE}ProbNNp,${TUNE}ProbNNpi,${TUNE}ProbNNe]" --cuts="[lab:${HBT_CUTS}]" > PIDdistribution.log
sleep 100
