...
int main(int argc, char** argv) {
  ...
  int MC = (argc>1 && !strcmp(argv[1], "MC")) ? 1 : 0;
  TString fileName = (argc>1 && !strcmp(argv[1], "MC")) ? TString("hbtNTuple_corrected.root") : TString("hbtNTuple.root");
  TString prod = (argc>2 && !strcmp(argv[2], "local")) ? TString("EPOS_local/") : TString("");

  TString f1_name = TString("/mnt/lustre/scratch2/people/plgbmalecki/BEC_pPb/ntuple/"+TString(argv[1])+"_"+TString(argv[3])+"/"+prod+fileName);
  TString label = TString(TString(argv[1])+"_"+TString(argv[2])+"_"+TString(argv[3]));
