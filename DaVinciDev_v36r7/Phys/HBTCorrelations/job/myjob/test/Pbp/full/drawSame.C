...
void drawSame1(TString f1Name,TString h1Name,TString f2Name,TString h2Name,TString title = "")
{
  ...
  tl->AddEntry(h1, "microBias" ,"pe");
  tl->AddEntry(h2, "FULL", "l");
...
void drawSame()
{
  TString path = TString("/afs/cern.ch/work/b/bmalecki/analysis/BEC_pPb/DaVinciDev_v36r7/Phys/HBTCorrelations/job/myjob/test/Pbp/");

  TString hName = TString("h101");
  TString title = TString("N_{ch};N_{ch};(1/N)(dN/dN_{ch})");
  drawSame1(TString(path+"microBias/HBTAnalysis/basicVars.root"),hName,TString(path+"full/basicVars.root"),TString("htemp"),title);
