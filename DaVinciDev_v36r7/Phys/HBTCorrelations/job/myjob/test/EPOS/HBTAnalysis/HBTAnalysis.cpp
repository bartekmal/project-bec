...
int main(int argc, char** argv) {
  ...
  const float nMax = 1000000; 
  
  // Choose particles to be analysed (1-pions, 2-kaons, 3-protons)
  int MC = 1; //local
  TString mode = TString("pPb_MU"); //pPb_MU
  TString label = TString("basicVars");

  if(argc>1)
  {
    if(!strcmp(argv[1], "central")) MC=0;
    if(!strcmp(argv[1], "local")) MC=1;
  }
  ...
  if(!MC)
  {
    f1_name = TString("/mnt/lustre/scratch2/people/plgbmalecki/BEC_pPb/ntuple/MC_"+mode+"/hbtNTuple_corrected.root");
    label = TString(label+"_central");
  }else
  {
    f1_name = TString("/mnt/lustre/scratch2/people/plgbmalecki/BEC_pPb/ntuple/MC_"+mode+"/EPOS_local/hbtNTuple.root");
    label = TString(label+"_local");
  }
  ...
  printf("000000.0001 Exec args: central/local (0/1) = %d\n",MC);
  printf("000000.0001 Exec args: mode = %s\n",mode.Data());
  

  TFile fHisto(TString(label) + ".root","RECREATE");
  // Insert histograms ----------
  TH1D* h100 = new TH1D("h100","trkMult", 150, 0.0, 150.0);
  ...
  TH1D* h1008 = new TH1D("h1008","ghostNN", 100, 0., 0.5);
